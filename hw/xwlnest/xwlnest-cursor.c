...
xwl_unrealize_cursor(DeviceIntPtr device, ScreenPtr screen, CursorPtr cursor)
{
    ...
    return xwlnest_shm_destroy_pixmap(pixmap);
}
...
xwl_seat_set_cursor(struct xwl_seat *xwl_seat)
{
    ...
    if (cursor->bits->argb)
        ...
        expand_source_and_mask(cursor, pixmap->data);
    ...
    wl_surface_attach(xwl_seat->cursor,
                      xwlnest_shm_pixmap_get_wl_buffer(xwl_seat->xwl_screen->shm, pixmap), 0, 0);
...
xwl_screen_init_cursor(vfbScreenInfoPtr xwl_screen)
{
    ...
    return miPointerInitialize(xwl_screen->pScreen,
